...
It will create a new file called `swap-card.tsx` inside the `components/mage-ui/card` directory.

```bash
mkdir -p components/mage-ui/card && touch components/mage-ui/card/swap-card.tsx
```
...
```jsx file=<rootDir>/mage-ui/card/swap-card.tsx
