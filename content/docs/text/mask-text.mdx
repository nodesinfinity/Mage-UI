...
It will create a new file called `mask-text.tsx` inside the `components/mage-ui/text` directory.

```bash
mkdir -p components/mage-ui/text && touch components/mage-ui/text/mask-text.tsx
```
...
```jsx file=<rootDir>/mage-ui/text/mask-text.tsx
