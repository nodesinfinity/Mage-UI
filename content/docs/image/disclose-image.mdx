...
It will create a new file called `disclose-image.tsx` inside the `components/mage-ui/image` directory.

```bash
mkdir -p components/mage-ui/image && touch components/mage-ui/image/disclose-image.tsx
```
...
```jsx file=<rootDir>/mage-ui/image/disclose-image.tsx
