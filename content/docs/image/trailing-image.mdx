...
It will create a new file `trailing-image.tsx` inside the `components/mage-ui/image` directory.

```bash
mkdir -p components/mage-ui/image && touch components/mage-ui/image/trailing-image.tsx
```
...
```jsx file=<rootDir>/mage-ui/image/trailing-image.tsx
