...
It will create a new file called `shining-button.tsx` inside the `components/mage-ui/button` directory.

```bash
mkdir -p components/mage-ui/button && touch components/mage-ui/button/shining-button.tsx
```
...
```jsx file=<rootDir>/mage-ui/button/shining-button.tsx
