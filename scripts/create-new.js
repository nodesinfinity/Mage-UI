...
const existingFolders = fs.readdirSync("mage-ui");
...
(async () => {
  ...
  if (!fs.existsSync(`mage-ui/${variables.TypePath}`)) {
    fs.mkdirSync(`mage-ui/${variables.TypePath}`);
    shouldRegister = true;
  }
  ...
  if (fs.existsSync(`mage-ui/${variables.TypePath}/${variables.Filename}.tsx`)) {
    console.log(
      chalk.red(`File "mage-ui/${variables.TypePath}/${variables.Filename}.tsx" already exists.`),
    );
    return;
  }
  ...
  createCopy("component", `mage-ui/${variables.TypePath}/${variables.Filename}.tsx`, variables);

  createCopy("story", `mage-ui/${variables.TypePath}/${variables.Filename}.stories.tsx`, variables);
